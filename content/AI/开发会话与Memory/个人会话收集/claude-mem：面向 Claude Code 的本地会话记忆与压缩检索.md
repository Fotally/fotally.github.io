...
> **项目快照**：官方仓库 <https://github.com/thedotmack/claude-mem>｜核验日期 2026-09-05｜Stars 约 93k｜许可证 Apache-2.0。最新发布版为 `v13.24.0`（2026-09-03）；README 已说明产品品牌改为 Grok Mem，但 npm 包名和 Claude Code 插件仍保留 `claude-mem`。本报告同时参考 2026-09-04 的 `main` commit `be44b6c`，版本标签与持续演进分支不能无条件视为同一版本。[^claude-mem-repository][^claude-mem-license][^claude-mem-release][^claude-mem-main-commit]
...
本地 Chroma 采用 `uvx` 启动的 `chroma-mcp` 子进程和 ONNX embedding 模型，部署轻量但带来 Python/uv/ONNX 运行时、首次模型下载、跨平台兼容和进程回收问题。官方源码将 `all-MiniLM-L6-v2` 与 `onnxruntime>=1.20` 固定在默认本地启动路径；项目没有直接配置 Embedding provider/model/base URL 的一等入口，但 Chroma 支持 remote mode，外部远程 Chroma 可能间接替换向量后端，不属于当前官方一键配置。[^chroma-manager][^embedding-customization][^claude-mem-chroma-remote]
...
| 会话排队 | 清理后的事件和会话 ID | Worker 通过本地 HTTP API 登记会话；当前活动观察由进程内 `SessionMessageBuffer` 缓冲，崩溃恢复依赖 transcript replay | `sdk_sessions`、`user_prompts` 与内存中的待处理消息；`pending_messages` 主要是兼容/旧路径 | [^worker-service][^database-architecture][^claude-mem-buffer][^claude-mem-transcript] |
...
| 向量语义检索 | 期望 | ChromaSync 调用本地或远程 Chroma MCP；默认使用 chroma-mcp 内置 ONNX embedding | [^chroma-sync][^chroma-manager][^claude-mem-chroma-remote] | 部分满足 | 语义检索需要 Python/uv/chroma-mcp 和模型初始化；当前没有直接配置 Embedding provider/model/base URL 的一等入口，远程 Chroma 可能间接替换后端但不属于官方一键配置。 |
...
生成模型切换和 Embedding 模型切换是两个不同边界。当前源码允许通过提供商设置改变“观察压缩/总结”调用；但默认本地 Chroma 的向量生成由 `chroma-mcp` embedding function 决定，源码固定依赖 `all-MiniLM-L6-v2` 的 ONNX 运行路径。项目没有直接配置 Embedding provider/model/base URL 的一等入口，远程 Chroma 可能间接改变后端，但仍需外部配置与迁移治理。[^configuration][^chroma-manager][^embedding-customization][^claude-mem-chroma-remote]
...
但本次目标还要求关注团队会话沉淀、Embedding API 切换、完整原始会话可追溯和用户确认上传。向量 Embedding 当前固定在 chroma-mcp 的本地 `all-MiniLM-L6-v2` ONNX 路径，生成模型的 API 切换不能解决这一问题；团队共享、审批上传和 Skill 自动更新也需要外部治理层。因此不能给出“直接匹配”。[^chroma-manager][^embedding-customization][^claude-mem-license]
...
- **Embedding 模型和 API 无直接配置入口**：当前默认是 `all-MiniLM-L6-v2` ONNX；没有官方 `CLAUDE_MEM_EMBED_PROVIDER`、模型、维度和 API Base URL 设置。远程 Chroma 可能间接替换 embedding backend，但需要外部配置和迁移治理。对中文业务知识或公司 Embedding API，召回质量和合规性都需要额外验证。[^chroma-manager][^embedding-customization][^claude-mem-chroma-remote]
...
- **Chroma 与活动队列的可靠性需要核验**：`uvx`、Python、ONNX 首次冷启动和跨平台进程生命周期可能影响 Hook 体验；当前内存缓冲在 Worker 崩溃时可能丢失未确认消息，transcript replay 是恢复边界；官方 Issue 也记录过语义搜索失败时回退 FTS 的情况。[^chroma-issues][^chroma-windows][^claude-mem-buffer][^claude-mem-transcript]
...
- 如果 Embedding API 切换是硬性要求，当前开源实现没有直接的 provider/model/base URL 配置入口；可采用本地默认 ONNX、关闭语义检索，或自行配置远程 Chroma/embedding backend。[^embedding-customization][^claude-mem-chroma-remote]
...
[^claude-mem-main-commit]: [claude-mem main 最新提交（截至 2026-09-04）](https://github.com/thedotmack/claude-mem/commit/be44b6c8e238a7e2bc5b3403c05afac071a59ead)
