...
- [[claude-mem：面向 Claude Code 的本地会话记忆与压缩检索]]：通过 Claude Code Hooks、本地 SQLite/FTS5 和 Chroma 检索保留开发上下文。
- [[Lunary：会话日志、反馈与开源边界案例]]：记录会话日志与反馈能力的开源边界，作为社区成熟度反例。
- [[Helicone：以 AI Gateway 代理和请求日志沉淀会话|Helicone]]：以统一 AI Gateway 代理模型请求并聚合会话日志。

## 统计

- 共 11 篇：10 个主清单项目（Entire CLI、CloudCLI UI、OpenLIT、Phoenix、Langfuse、AgentOps、Opik、OpenLLMetry、claude-mem、Helicone），以及 1 个开源/社区边界案例（Lunary）。
