...
Mem0 的核心定位是把消息提炼成可检索的长期事实，而不是保存完整 Agent 开发轨迹。当前 OSS 数据流大致为：`add(messages)` → LLM 提取事实 → 实体与 Embedding 生成 → 写入向量存储及历史记录 → `search` 进行语义、BM25、实体匹配与融合排序。自动提取时主要只新增事实，不自动修改或删除旧事实；需要修改或删除时，由应用或管理员通过显式 API 完成。核心编排在 `mem0/memory/main.py`，Provider 工厂在 `mem0/utils/factory.py`。[^mem0-readme][^mem0-core]

它用 `user_id`、`agent_id`、`run_id` 表示作用域，适合映射员工、Agent 和会话。SDK 的 LLM、Embedding 与向量库分别可配，Provider Factory 包含 DeepSeek、Ollama、vLLM、LiteLLM 等；但官方 Self-hosted Server 镜像默认只打包有限的 LLM / Embedder Provider。DeepSeek、LiteLLM 或公司 OpenAI-compatible 网关需要按官方配置和部署版本单独核验；必要时才扩展 Server 白名单、加入依赖并重建镜像。官方 Server Compose 是 FastAPI、PostgreSQL/pgvector 和 Dashboard 三个服务，一台服务器可承载。[^mem0-config][^mem0-server-providers][^mem0-compose]
...
**问题与设计**：Hindsight 用 `Retain` 将对话、文档和 Agent 事件提取为事实、实体、关系、时间和经历，按 Memory Bank 隔离；`Recall` 做事实/经历召回，`Reflect` 基于召回结果综合回答。**工作流与产物**：后台继续整理观察和心理模型，输出可召回的事实、经历与反思结果。**适配与边界**：适合研究“记住之后继续学习”的 Memory，可依赖 API、PostgreSQL/pgvector 和 LLM/Embedding；本地会话发现、上传审批、原始回放和 Skill 治理仍需外部补齐。详见 [[Memory/Hindsight：Retain、Recall、Reflect 的学习型 Agent Memory]]。[^hindsight-operations]

#### 6. EverOS：Markdown 事实源与 Skill 演化

**问题与设计**：EverOS 将开发会话中的经验保留为带 frontmatter 的 Episode Markdown，并以可读文件作为事实源。**工作流与产物**：Cascade/OME 异步派生 AtomicFact、Foresight、Profile、Agent Case 和 `SKILL.md`，SQLite/LanceDB 只承担可重建的元数据与混合检索索引。**适配与边界**：最贴近“可读 Memory → Git 化 Skill”，但运行成熟度、团队权限和跨 Agent 接入仍需验证，Memory/Skill 发布门禁需外部治理。详见 [[Memory/EverOS：以 Markdown、Episode 和 Skill 演化的本地 Memory]]。[^everos-how-memory][^everos-ome]

#### 7. Supermemory：事实演化图与混合检索

**问题与设计**：Supermemory 将文档或会话抽取为事实，处理更新、矛盾与遗忘，形成 Profile，并以语义、关键词和图信号混合检索；`containerTags` 可表达项目范围。**工作流与产物**：输入经事实演化后输出 Profile、Memory 和混合检索结果，官方列出的客户端包括 Claude Code、OpenCode、OpenClaw、Hermes、Cursor、Windsurf 和 VS Code；性能基准中的 Codex 不等于现成集成。[^supermemory-readme] **适配与边界**：Local 自托管版是单二进制和单数据目录，支持多种模型/兼容 Base URL；默认 `bge-base-en-v1.5` 偏英文。公开主仓为 MIT，但发布版 `supermemory-server` 的核心引擎源码边界仍需确认，本地版也是单租户/单组织，团队角色和细粒度 Key 属于外部或 Enterprise 边界。详见 [[Memory/Supermemory：事实演化、用户画像与混合检索 Memory]]。[^supermemory-local][^supermemory-provider][^supermemory-enterprise][^supermemory-server-source]

#### 8. LightRAG：图、向量与关键词三路检索

**问题与设计**：LightRAG 将文档或会话切成文本块，抽取实体和关系写入知识图，同时为文本块、实体和关系建立 Embedding 与关键词索引。**工作流与产物**：查询按 local/global 模式融合图、向量和关键词结果后交给 LLM，输出带知识关联的回答。**适配与边界**：适合作为业务知识检索底座，可组合 REST/API、图存储和向量后端；它不是完整 Agent Memory，用户画像、记忆遗忘、会话采集和 Skill 治理需要外部实现。详见 [[Memory/LightRAG：图结构与向量检索的业务知识 Memory]]。[^lightrag-readme][^lightrag-operate]

#### 9. Memobase：Profile、Event 与缓冲 flush

**问题与设计**：Memobase 以用户/项目为范围，把输入暂存为 Blob buffer，并用结构化 Profile 与 Event Timeline 控制记忆粒度。**工作流与产物**：达到 token、空闲或显式 `flush` 条件后，LLM 按 `topic/sub_topic` 生成 Profile Delta，合并为 `UserProfile`，并保存带时间的 `UserEvent`/`Event Gist`；在线通过 Profile、Event 或 Context API 取结果，事件语义检索才需要 Embedding。**适配与边界**：适合受控的项目知识槽位和事件时间线，可依赖 API、PostgreSQL/pgvector 与 Redis；多 Agent 会话适配、消息级 provenance、权限和 Skill 流程要由外围补齐。详见 [[Memory/Memobase：以 Profile 与事件时间线构建可控用户 Memory]]。[^memobase-repository][^profile-fundamentals]

#### 10. MemU：Agent 写作、MemoryService 检索

**问题与设计**：MemU 在 host bridging 和 developer memorize 的自演化阶段把“理解和写作”交给外部 Agent，把 Markdown 事实或 Skill 交给轻量 `MemoryService` 保存、向量化和检索。**工作流与产物**：宿主 adapter 或 `MemorizeInput` 进入 `prepare → Agent → commit`，输出可读 memory/skill 文件、RecallFile/Segment 和索引；MemoryService 本身是 embedding-only，但 v2 包另有 LLM/VLM 处理组件。**适配与边界**：它非常贴近 Skill 候选生成，可用 SQLite 或 PostgreSQL/pgvector；原始 transcript 归档、权限、审计和人工发布仍需外部治理。详见 [[Memory/MemU：面向主动记忆与多 Agent 的记忆编排]]。[^memu-service][^memu-readme][^memu-developer]

#### 11. Zep：Context Graph 的托管边界案例

**问题与设计**：Zep Cloud 以用户、线程、Episode/Observation 和时间知识图谱组装上下文。**工作流与产物**：托管 API 输出按线程和时间组织的 Context Graph 上下文；当前 `getzep/zep` 仓库主要是 Cloud 示例、集成和导入工具。**适配与边界**：仓库不包含可直接自托管的当前 Context Graph Engine，Community Edition 已进入 legacy；它适合借鉴图谱产品形态，但不满足单机内网硬约束，实际开源实现应看 Graphiti。详见 [[Memory/Zep：托管 Context Graph 生态与开源边界案例]]。[^zep-boundary]
...
**问题与设计**：Opik 把 Agent 执行组织成可查询 Trace 树，将失败观察转为数据集和可重复评估。**工作流与产物**：Trace → annotation/dataset → evaluation → CI feedback，形成可回归的评估结果。**适配与边界**：适合连接会话问题与 Skill/提示修改的回归验证，可依赖 API/UI、数据库和评估任务组件；仍需外部接入本地 transcript、原始证据和审批。详见 [[个人会话收集/Opik：Agent Trace、评估和 CI 反馈沉淀开发经验]]。[^opik-repository]

#### 7. AgentOps：Session Replay 与 Agent Span

**问题与设计**：AgentOps 以 `SESSION` 为根，组织 `AGENT`、`WORKFLOW`、`TASK`、`LLM` 和 `TOOL` Span。**工作流与产物**：事件经 OTLP 后端进入 Dashboard，形成 Session Replay、统计和分析记录。**适配与边界**：适合作为执行过程可视化参考，可依赖 API、Supabase、ClickHouse 和 OTel Collector；没有现成的多 Agent 本地历史解析器，SDK 与自托管 app 的许可证边界也需分开核对。详见 [[个人会话收集/AgentOps（以 Session Replay 和 Agent Span 记录开发经验）]]。[^agentops-selfhost]

#### 8. OpenLLMetry：可替换的 OTel 采集层

**问题与设计**：OpenLLMetry 用 OpenTelemetry 语义属性表达模型、Token、提示词、响应、工具和检索信息，解耦采集层与后端。**工作流与产物**：Agent SDK instrumentation 产生 spans，经 OTel Collector 发送到任意后端，形成可查询 Trace。**适配与边界**：它是统一 Instrumentation 层而非会话管理或 Memory 平台，可依赖 SDK、Collector 和后端；会话选择、原始归档、回放和中央评审需另行组合。详见 [[个人会话收集/OpenLLMetry：以 OpenTelemetry Instrumentation 作为跨 Agent 采集层]]。[^openllmetry]

#### 9. claude-mem：观察压缩与渐进式检索

**问题与设计**：claude-mem 的 legacy 本地 Worker 通过 Claude Code Hook 捕获工具事件，异步压缩为标题、叙事、事实、文件和概念。**工作流与产物**：SQLite/FTS5 保存结构化记录，Chroma 可选；MCP 采用索引、时间线、详情的渐进式检索，SessionStart 再注入上下文。**适配与边界**：适合个人 Claude Code 连续使用；公开 Server-beta、Cloud Sync 和多 Agent 集成需与 legacy 本地路径分开评估，完整原始 transcript、团队治理和 Skill 发布仍需外部补齐。详见 [[个人会话收集/claude-mem：面向 Claude Code 的本地会话记忆与压缩检索]]。[^claude-mem-architecture][^claude-mem-server][^claude-mem-cloud]

#### 10. Helicone：模型请求边界的 Gateway 日志

**问题与设计**：Helicone 在模型请求边界提供 Gateway、日志、路由、成本与 Session 分析，通常通过 base URL 或 header 接入。**工作流与产物**：请求/响应经 Gateway 进入日志、Session 和成本分析后端。**适配与边界**：适合作为模型调用观测层，可依赖 Gateway、Postgres、ClickHouse、MinIO 和 Redis；不天然记录终端、文件修改、测试结果或本地会话发现，因此不能替代完整开发会话证据层。详见 [[个人会话收集/Helicone：以 AI Gateway 代理和请求日志沉淀会话|Helicone]]。[^helicone-sessions][^helicone-selfhost]

#### 11. Lunary：会话、执行记录与反馈边界案例

**问题与设计**：Lunary 将可观测性拆成 `thread/message`、`run/trace` 和 `feedback`，通过 SDK 上报后在控制台消费。**工作流与产物**：输入形成线程、执行记录和人工反馈，供基础会话分析使用。**适配与边界**：当前公开代码仓库和历史官方仓库的身份、社区规模与维护状态存在边界，因而仅作为开源/社区对照案例；它不应被视为可独立完成本地 Agent 采集、Memory 提炼和 Skill 治理的方案。详见 [[个人会话收集/Lunary：会话日志、反馈与开源边界案例]]。[^lunary-repository]

### 项目实施限制概览

不同项目的部署构成应按职责理解，而不是按单一重量排序：Memory 项目通常需要运行时、LLM/Embedding 和 SQLite、PostgreSQL/pgvector 或图数据库；会话采集项目通常需要本地 Hook/CLI、OTel Collector 或 Worker，再按需要增加中央数据库、ClickHouse、对象存储或评估 UI。Zep 和 Lunary 的托管/边界能力不应计入自托管主路径。以上归纳来自各项目报告的部署章节，不能替代容量测试或官方容量承诺。

## 3. 从项目中归纳出的主流设计思想

### 原始证据与派生知识分层

Entire CLI 和 CloudCLI UI 体现了先在本地发现、预览或保存开发会话，再把经授权的材料交给分析层的思路；Graphiti、Cognee、MemU 和 EverOS 则说明 Memory 应保存为可追溯的事实、关系、Episode 或可读文件。由此可见，原始 transcript、标准化事件、派生 Memory 和候选 Skill 应使用不同的数据模型与授权边界，而不能用一条摘要替代原始证据。[^entire-session-checkpoint][^cloudcli-providers][^graphiti-core][^memu-service][^everos-how-memory]

### 标准化采集与渐进式检索

OpenLIT、OpenLLMetry、Phoenix 和 Langfuse 共同体现了用 OpenTelemetry/OpenInference 统一 Agent、LLM、Tool、Retriever 和 Evaluator 事件，再由中央系统查询、回放和评估的路径。claude-mem、Mem0 和 Graphiti 进一步体现了先返回紧凑索引或混合检索结果，再按需取回详情、事实或关系的渐进式上下文策略。该思想适合控制上下文成本，但标准化 span 仍需保留原始事件偏移和项目语义，不能假设不同 Agent 的属性天然等价。[^openinference-spec][^openlit-telemetry][^openllmetry][^phoenix-sessions][^langfuse-model][^search-architecture][^mem0-core][^graphiti-search]

### 可读知识与人工治理共同演化

Letta Code 的 Git-backed Memory、EverOS 的 Markdown/Skill 派生链和 MemU 的 Agent 主导写作，都把记忆或 Skill 保留为可读、可 diff 的资产；Langfuse、Phoenix 和 Opik 则提供回放、标注、数据集或评估所需的反馈面。它们共同支持“经验形成候选变更，但必须经过人工评审、Git 合并和回归验证”的治理原则；这是一条由多个项目归纳出的方案，不是任何单个项目的完整内置闭环。[^letta-memory-git][^everos-ome][^memu-agentic][^langfuse-datasets][^phoenix-export][^opik-repository]

### 补充对照与边界项目

以下项目保留用于理解相邻方案或开源边界，不改变两个主清单各十个项目的范围：
...
| Helicone | 模型 gateway 和 request/response 分析强，但不会自动记录终端、文件修改与测试；完整 Compose 还包含多个基础设施服务，作为模型请求观测补充，不替代本地会话采集。[^helicone-sessions][^helicone-selfhost] |
| AgentOps | Agent session 语义较强，但自部署依赖多个服务，且没有现成的 Claude/Codex/Cursor 本地历史解析器，作为 Session Replay 参考。[^agentops-selfhost] |
| claude-replay | 直接读多 Agent 日志并提供 HTML/JSONL 回放与 secret redaction，作为回放与脱敏 UX 参考。[^claude-replay] |
| OpenLLMetry | 是有价值的 OTel instrumentation 库，不是会话管理/评审平台，因此作为采集层组件而非完整平台。[^openllmetry] |
