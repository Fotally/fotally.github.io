...
本专题已补齐两组各 10 个主清单项目报告，并额外保留 Zep、Lunary 两个开源边界案例：Memory 报告见 [[Memory/index|Memory 项目索引]]，个人会话收集报告见 [[个人会话收集/index|个人会话收集项目索引]]。下文保留用于分享的横向机制与组合视角，逐项目事实以对应报告为准。

本轮还为 10 篇 Memory 报告补充了“向量化与模型接口核验”小节，并在 Memory 索引提供统一速查表，明确 Embedding 是否必需、官方示例模型、向量后端、OpenAI-compatible/DeepSeek 兼容条件以及更换模型后的重建索引要求。
...
3. 18 分钟：10 个 Memory 项目的不同数据模型与部署边界。
4. 18 分钟：10 个会话项目从本地采集、Git provenance 到中央 eval 的分工。
5. 6 分钟：三种组合及选择条件。
